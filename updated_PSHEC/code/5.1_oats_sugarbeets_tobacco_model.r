...
# 3) use bootstrap to generate 95% prediction interval 
...
for (crop in c('OATS','SUGARBEETS','TOBACCO')) {
  ...
  model_function <- function(i) {
    ...
    basic_rec <- 
      recipe(Yield_decomp_add ~ 
               ...
               FarmLbsN_perAc_mean + # add nitrogen input
    ...
    lasso_corr_d_int2 <- 
      ...
      step_normalize(has_role("numeric_original") | starts_with("state_alpha_")|starts_with("ssurgo_order_mode_")|starts_with("ssurgo_drainage_class_")) %>%
      # interactions
    ...
    cl <- makePSOCKcluster(parallel::detectCores(15))  # Use defined number of core
