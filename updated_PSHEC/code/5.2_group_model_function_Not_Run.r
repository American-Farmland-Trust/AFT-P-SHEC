...
# This script is to run grouped crop models
# grouped model results were not used in the final yield prediction
...
group_function <- function(...) {
  ...
  output_dir <- paste0('/home/meng/Documents/updated_PSHEC/models/',group_name,'_group_model/')
  ...
  data_list<- lapply(crop_name, function(crop) {
    ...
    df <- readRDS(paste0('~/Documents/updated_PSHEC/merged_data/',crop,'_all_data_soil_weather_N_n15_om10_AFT.rds'))
  ...
  #model_function <- function(i) {
    ...
    # print the progress
    cat("Start the workflow for:", group_name, "\n")
    ...
    cl <- makePSOCKcluster(parallel::detectCores()-4)  # Use defined number of core
    ...
    saveRDS(race_results_filtered, paste0(output_dir,group_name,'_race_results.rds'))
    ...
    ggsave(model_rmse, filename = paste0(output_dir,group_name,'_model_selection.png'))
    ...
    saveRDS(final_rf_fit, file = paste0(output_dir,group_name,'_model_selection_fit_objective.rds'))
    ...
    corn_test_list<- lapply(crop_name, function(crop) {
      
      corn_test_select <- corn_test %>% filter(crop == crop)
      
      all_predictions_test <- bind_rows(
        augment(final_rf_fit, new_data = corn_train) %>% 
          mutate(type = "train"),
        augment(final_rf_fit, new_data = corn_test) %>% 
          mutate(type = "test"),
        augment(final_rf_fit, new_data = corn_test_select) %>% 
          mutate(type = paste0("test_",crop)))
    ...
    rmse_combine <- dplyr::bind_rows(corn_test_list)

    
    
    write.csv(rmse_combine, file = paste0(output_dir,group_name,'_model_selection_metrics.csv'))
...
groups_list <- list(
  c("BARLEY", "OATS"),
  c("BARLEY", "OATS", "SORGHUM"),
  c("PEANUTS", "SUGARBEETS", "TOBACCO")
)

# Loop through each group and call group_function
lapply(groups_list, function(group) {
  do.call(group_function, as.list(group))
})
