...
gdb_list <- list.files("/home/shared/Data_updates_Jan2024/gSSURGO by State/", full.names = T, pattern = "gdb")
...
nass_15 <- read.csv("/home/shared/Data_updates_Jan2024/all_10_crops_county_list_2000_2023.csv")
...
output_dir <- 'gSSURGO/all_states_gssurgo/'
...
all_states_gssurgo <- parallel::mclapply(mc.cores = 28, gdb_list, function(i){
  ...
  temp3 <- sf::st_read(i, layer = "component") %>%
    dplyr::select(comppct_r, taxorder,drainagecl,slope_r, cokey, mukey,majcompflag) 
  ...
  temp4 <- temp3 %>%
    ...
    mutate(order = taxorder[which(majcompflag %in% "Yes")[1]],
           drainage_class = drainagecl[which(majcompflag %in% 'Yes')[1]]) %>%
    summarize(clay = weighted.mean(SSURGO_claytotal_r, w = comppct_r, na.rm = TRUE),
              ...
              slope = weighted.mean(slope_r, w = comppct_r, na.rm = TRUE), # add slope and calculate weighted mean
              order = unique(order),
              drainage_class = unique(drainage_class)) %>% # add drainage class
    janitor::clean_names(.)
...
write_rds(all_states_gssurgo, 
        file = paste0("gSSURGO/all_states_gssurgo/all_states_gssurgo_n_15_part",n,".rds"))
...
```{r list}
# Define the output directory
output_dir <- 'gSSURGO/all_counties_gssurgo/'
...
file_list <- list.files(path = 'gSSURGO/all_states_gssurgo/','rds',full.names = TRUE)

for (i in 1:length(file_list)) {

all_states_gssurgo <- readRDS(
  paste0("gSSURGO/all_states_gssurgo/all_states_gssurgo_n_15_part",i,".rds"))
...
write_rds(all_counties_gssurgo, 
          file = paste0("gSSURGO/all_counties_gssurgo/all_counties_gssurgo_part",i,".rds"))
...
# Define the output directory
input_dir  <- "gSSURGO/all_counties_gssurgo/"
temp_dir   <- "gSSURGO/county_gssurgo_temp_chunks/"
output_dir <- 'gSSURGO/county_gssurgo_gdbs/'
...
file_list <- list.files(path = 'gSSURGO/all_counties_gssurgo/','rds',full.names = TRUE)

#for (i in 1:length(file_list)) {

#all_counties_gssurgo <- read_rds(file = paste0("gSSURGO/all_counties_gssurgo/all_counties_gssurgo_part",i,".rds"))

# Error note: this code can generate partial files because write.rds is within the loop
...
#            file = paste("gSSURGO/county_gssurgo_gdbs/GEOID_", x, ".rds", sep = ""))
#})

target_crs <- sf::st_crs(5070)

# -----------------------------
# Step 1: Split each part file into temporary GEOID chunks
# -----------------------------

parallel::mclapply(seq_along(file_list), mc.cores = 4, function(i) {
  
  message("Processing part ", i)
  
  all_counties_gssurgo <- readr::read_rds(file_list[i]) %>%
    dplyr::mutate(GEOID = as.character(GEOID))
  
  if (inherits(all_counties_gssurgo, "sf")) {
    sf::st_crs(all_counties_gssurgo) <- target_crs
  }
  
  county_list <- split(all_counties_gssurgo, all_counties_gssurgo$GEOID)
  
  purrr::iwalk(county_list, function(df, geoid) {
    readr::write_rds(
      df,
      file.path(temp_dir, paste0("GEOID_", geoid, "_part", i, ".rds"))
    )
  })
  
  rm(all_counties_gssurgo, county_list)
  gc()
...
# -----------------------------
# Step 2: Combine temporary chunks by GEOID
# -----------------------------
...
geoids <- stringr::str_extract(
  basename(chunk_files),
  "(?<=GEOID_)\\d+"
)

unique_geoids <- unique(geoids)

parallel::mclapply(unique_geoids, mc.cores = 4, function(geoid) {
  
  message("Combining GEOID ", geoid)
  
  geoid_files <- chunk_files[geoids == geoid]
  
  county_data_list <- lapply(geoid_files, function(f) {
    ...
    # Detect active geometry column
    geom_col <- attr(x, "sf_column")
    
    # Rename geometry column to a standard name
  ...
  # Use base rbind for sf objects
  county_data <- do.call(rbind, county_data_list)
  
  sf::st_crs(county_data) <- target_crs
  
  readr::write_rds(
    county_data,
    file.path(output_dir, paste0("GEOID_", geoid, ".rds"))
  )
  
  rm(county_data, county_data_list)
  gc()
